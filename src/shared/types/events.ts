...
export interface MutationEvent extends Event {
  ...
  data: {
    ...
    groupId: string;
    role: Role;
    mutation: any; // Will be replaced with IMutation when we migrate that type
...
export interface EvolutionTrackedEvent extends Event {
  ...
  data: {
    ...
    blockNumber: number;
    confirmations: number;
    mutations: any[]; // Will be replaced with IMutation[] when we migrate that type
    timestamp: number;
  };
}
...
export interface FormationEvent extends Event {
  ...
  data: {
    ...
    groupId: string;
    role: Role;
    formation: any; // Will be replaced with IFormation when we migrate that type
  };
}
...
export interface BehaviorEvent extends Event {
  ...
  data: {
    ...
    groupId: string;
    role: Role;
    behavior: any; // Will be replaced with IBehavior when we migrate that type
  };
}
...
export interface AbilityEvent extends Event {
  ...
  data: {
    ...
    groupId?: string;
    role?: Role;
    ability: any; // Will be replaced with IAbility when we migrate that type
    target?: string;
  };
}
...
export interface GameTheoryEvent extends Event {
  ...
  data: {
    matrixId: string;
    matrix: any; // Will be replaced with IPayoffMatrix when we migrate that type
    players: string[];
    equilibria?: any[]; // Will be replaced with INashEquilibrium[] when we migrate that type
    optimalStrategy?: any; // Will be replaced with IStrategyProfile when we migrate that type
